...
class Predictor(object):
    def __init__(self, num_classes, checkpoint_path='./checkpoint'):
        ...
        self.images_batch = tf.placeholder(tf.float32, shape=[self.num_thetas, 227, 227, 3])
        self.indicators_batch = tf.placeholder(tf.float32, shape=[self.num_thetas, 1])

        self.model = GraspNet(self.num_classes)
...
if __name__ == '__main__':
    ...
    import cv2
    NUM_THETAS = 1
    p = FCPredictor(NUM_THETAS*2, './Network1-1000-100')
    img = Image.open('./img.png')
    
    img_arr = np.array(img)
    img_arr = cv2.resize(img_arr, (1280, 720), interpolation=cv2.INTER_CUBIC)
